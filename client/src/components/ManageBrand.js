...
class ManageBrand extends Component {
    ...
    onBtnAddClick = () => {
        if(document.getElementById("AddBrandImage").files[0] !== undefined) {
            var formData = new FormData() // FormData() ini bawaan,, sama kayak new Date()

            //untuk add file, header type nya harus {'Content-Type': 'multipart/form-data'}
            var headers = {
                headers: 
            ...
            if(document.getElementById('AddBrandImage')){
                //append mirip kayak push,, append ini sintax bawaan dari FormData
                formData.append('image', document.getElementById('AddBrandImage').files[0])
            }
            formData.append('data', JSON.stringify(data))

            axios.post("http://localhost:1997/addbrand", formData, headers)
            .then((res) => {
    ...
    onBtnDeleteClick = (id) => {
        if(window.confirm('Are you sure to delete?')) {
            axios.delete('http://localhost:1997/deletebrand/' + id)
            .then((res) => {
    ...
    onBtnUpdateClick = (id) => {
        ...
        axios.put("http://localhost:1997/editbrand/" + id, formData, headers)
        .then((res) => {
        ...
        var listJSX = this.state.brandList.map((item) => {
            ...
            return (
                <tr>
                    <td>{item.id}</td>
                    <td>{item.nama}</td>
                    <td><img src={`http://localhost:1997${item.image}`} alt={item.nama} width={100} /></td>
